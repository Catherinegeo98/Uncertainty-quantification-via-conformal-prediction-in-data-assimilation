...
def sweq(unoise,k,temp):
    ...
    r = np.where(r<0.,0.,r)#-non negaitivity omitted
    ...
    for it in range(nsub):   
      ...
      phi[0,1:nx+1,:] = np.where( h[1,1:nx+1,:] > h_cloud , phic, g*h[1,1:nx+1,:] ) 
      phi[0,0,:]   = phi[0,nx,:]
      phi[0,nx+1,:] = phi[0,1,:]
      ...
      r[2,1:nx+1,:] = r[0,1:nx+1,:] - alpha*dt*2*r[1,1:nx+1,:]-2*beta_new[1:nx+1,:]*(dt/dx)*(u[1,2:nx+2,:]-u[1,1:nx+1,:]) + (kr/(4*dx*dx))*(r[0,2:nx+2,:] - 2*r[0,1:nx+1,:] + r[0,0:nx,:])*dt   
...
def noise_u(r4,k):    
    ...
    ngauss = 1#r3.poisson(1,1)  
    for t in range(nsub):
        for i in range(ngauss):
            for j in range(k):
                pos= r4.randint(0,nx-1)  
