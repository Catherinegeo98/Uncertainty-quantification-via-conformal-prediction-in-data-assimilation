...
CQR_CP_PATHS = [
    r"/home/mga5500/Schreibtisch/PYTHON/cqr_halfwidth.pkl"
]
...
def cp_for_cycle(scores, ti):
    ...
    return {
        'u': np.asarray(entry.get('u', 0.0)),
        'h': np.asarray(entry.get('h', 0.0)),
        'r': np.asarray(entry.get('r', 0.0)),
    }
...
def add_cp_random_interval_scalar(E, cp, seed=None):
    ...
    cp_vec[0:nx_local]            = cp['u']
    cp_vec[nx_local:2*nx_local]   = cp['h']
    cp_vec[2*nx_local:3*nx_local] = cp['r']
    ...
    for ti in tqdm(range(cyc-1)):
        ...
        for method in methods:
            ...
            if method == 'NN' and ti >= 20 and 'EnKF' in methods:
                nn_bg = np.copy(data['EnKF']['analysis'][ti])
