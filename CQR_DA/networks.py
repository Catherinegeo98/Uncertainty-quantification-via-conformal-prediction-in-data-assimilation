...
def bias_loss(y_true, y_pred, ax):
    true_sum = tf.reduce_mean(y_true, 1)[:, ax]
    pred_sum = tf.reduce_mean(y_pred, 1)[:, ax]
    bias = tf.reduce_mean(tf.sqrt(tf.square(true_sum - pred_sum)))
    return bias
...
def combined_bias_loss(axs, betas):
    ...
    def combined_loss(y_true, y_pred):
        loss = tf.reduce_mean(tf.sqrt(tf.reduce_mean(tf.square(y_true - y_pred), axis=1)), axis=-1)
